...
OWNER_A_CONVOS = [
    (
        "开源发布检查",
        [
            _turn("林知远", "发布前必须完成许可证扫描、生产构建和无密钥端到端测试。", 20),
    ...
    (
        "Atlas 检索实验",
        [
            _turn("林知远", "The hybrid retrieval experiment combines BM25 and vector search.", 21),
...
async def main() -> int:
    ...
    failures: list[str] = []
    try:
        owner_a = UserId(f"u-opc-lab-a-{RUN}")
        owner_b = UserId(f"u-opc-lab-b-{RUN}")
