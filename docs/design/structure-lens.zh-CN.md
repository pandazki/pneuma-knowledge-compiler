...
Finding
  ...
  evidence[]   简短的逐字字符串或计数，≤ 5 条，每条 ≤ 200 字符
  impact       提示词目录键 + 字段   （它让人付出什么）
  action       提示词目录键 + 字段   （该做什么，说给 `actor` 听）
...
`impact` 与 `action` 是带具名字段的提示词目录键（`lens.<lens>.impact`、`lens.<lens>.action`），由 `prompt()` 渲染——于是中文覆盖能翻译它们，应用能改写它们，而同一个 ref 下的字节是稳定的。控制台用自己的 i18n 表渲染同样这些键，键名完全一致。

### 3.3 分数

一个 Owner 可以在两次快照之间盯着看的数字，仅此而已：`100 × (1 − Σ weight of open findings, capped at 1)`，其中 principle 发现的权重加倍。它不是打分；对比页签展示它的变化量，连同推动它的那些发现计数。

### 3.4 顺序

principle 在 drift 之前，drift 在 shape 之前；同一级别内按 `weight` 降序；再按 lens id；再按路径。控制台的头条是前三条。
...
`nav.dead_end` 与 `nav.arrival_blind` 留着是为了计数与对比页签；它们自己说不出欠的是哪一条链接。那四个契约角色透镜才是 action 点得出页面的那几个。
