...
| `pkc canonical ls` (`--include-archived`) / `read <path> [<path> …]` / `history <path>` | pages, a page, a claim chain; each read shows `last changed` (path commit), ledger claims as counted by outline, overview blocks, repository-wide supersession and successor addresses, `latest cited source` (latest cited block day, else source day), and pending + failed compile jobs for this tenant. The source index lists cited blocks with speakers and their own recorded days; missing authorship is `unknown`. Source dates use occurrence days, else `imported …`; `read` and `history` are unconditional; all dates use the Owner calendar declared in the header, explicitly UTC when unstated; JSON retains days and original offset timestamps. Block dates/speakers require matching envelope lengths, identifiers and block order, else unknown; email blocks expose sender, role and sent day |
