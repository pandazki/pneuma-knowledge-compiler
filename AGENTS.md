...
**Do not hand-edit a user's canonical git library (`data/canonical/<user>/`) and leave it uncommitted.** The git adapter recovers a dirty tree only when its own in-flight marker (`.git/pneuma.inflight`) proves the mess is a dead writer of its own; anything else is somebody's work and is REFUSED, not discarded — the job fails with `canonical_dirty:<paths>` and a request path that commits answers `409 canonical_dirty`. Commit, stash or revert what you changed there and the next run proceeds.
...
- `personal/` — the personal edition and `pkchome`, a standalone uv project with its own environment and committed lockfile, outside the root workspace. It consumes only the core/service library packages and its own assets; it never reads another application's source, and the library never imports it. Run commands from the worktree root with `uv run --project personal …`; its keyless, Docker-less tests are `uv run --project personal pytest personal/tests -q`.
...
**Ingest pipeline**: SourceAdapter (the only layer allowed to grow with input types) → NormalizedSource → IntakePlan (`canonical_treatment` × `semantic_indexing`, archetypes in core `domain/intake.py`) → index/compile jobs. The official input boundary is six versioned provider-neutral contracts: `meeting/v1`, `document-library/v1`, `im/v1`, `email/v1`, `owner-dialogue/v1` (the library owner's own statement — an ordinary source, no privileged write path), and `agent-session/v1` (Owner and coding-agent turns, with bounded activity stubs) ([docs/reference/source-contracts.md](docs/reference/source-contracts.md)).
...
**The archive** is how knowledge leaves every default retrieval without leaving the library: a canonical document is moved under the reserved root `archive/` (with its closed volumes; `git mv`, history intact), a source gets `archived_at` on L0. Both are authoritative marks; every derived index carries `archived` as a field re-derived on rebuild, every lane/listing takes `include_archived` (default false) and labels what it admits, the compile model never sees archived documents, and writes under `archive/` or at a shadowed live path are refused at the tool face and the gate (`archived_path`). An archived document leaves an **archive record** at its live path, written mechanically by the same commit (pure core `archive/record.py`, own narrow channel and own gate, no model): what the subject was, its span, how much it held, and the Owner's reason citing an `owner-dialogue/v1` statement the job ingests with `canonical_treatment: none`. That statement is always the Owner's OWN words — the framework composes none, because the source it writes is L0 labelled as the Owner speaking. They are also only ever the words the CONFIRM carried, because that request is the decision this statement records: a confirm with neither a note nor a `statement_ref` is refused (`422 note_required`), a note left on the proposal is no fallback (the plan requires none and quotes none), the job refuses defensively (`statement_missing`), and the console answers the friction by prefilling its own box with a suggestion the Owner edits and sends with the confirm. The record is LIVE — glance, claims, every lane by default — and READ-ONLY to compile, so the subject goes on being answered *as archived* instead of vanishing. An archive is computed as a **proposal** (citation closure plus the record's facts, pure core `archive/proposal.py`), confirmed by the Owner against the same canonical HEAD, and executed by the worker's `archive` job. Design authority: [docs/design/archive.md](docs/design/archive.md). **The word `archive`/`归档` is reserved for this and nothing else.** Rollover's `<doc>/aNN.md` files are **closed volumes** of a work in several volumes (the page itself being the **open volume**) — live knowledge every lane reads; never call them archives. The only legacy spellings kept on disk are the frontmatter key `archived_from` and the fallback `type: archive` a volume carries, both stamped into every user's git library already.
...
**Coding agent mode**: the compile role may name `agent:codex` / `agent:claude-code` instead of a model. A coding agent under the Owner's subscription then drives the round through the framework's CLI, `pkc` — reads at every layer, canonical writes only through `pkc draft`, whose draft lives in Postgres between commands, is post-checked with the gate's predicates on every write, and is finished by the same gate. The agent is taught by a generated skill package (`pkc skill install`, rendered from the prompt catalog and hashed into the commit trailer as `Executor-Skill`); the worker launches the harness headless for queued jobs, and the console's Owner lens has a Steward view over the same session. Design authority: [docs/design/coding-agent-mode.md](docs/design/coding-agent-mode.md); code under `packages/pneuma-knowledge-service/src/pneuma_knowledge_service/{cli,coding_agent}/`.

**Async everything**: import requests only write L0 + enqueue `index`/`compile` jobs; the worker processes them per-user serially **per lane** (PG `FOR UPDATE SKIP LOCKED`) and self-heals stuck jobs on restart. Two lanes (`service/job_lanes.py`): the **canonical lane** is every kind that can write the library (compile, evolve, evolve_adopt, groom, archive, challenge) and its one-in-flight rule *is* the git single-writer guarantee, unchanged; the **derived lane** (index, episodes, the recall projections) never opens canonical and is a second slot, so one writer and one derived job run at once. An unclassified kind is canonical, so forgetting the table costs parallelism and never the writer. API and worker are stateless. But **a function that awaits nothing is not a coroutine** — spine/gate/patch/projection/domain/render helpers stay sync; unavoidably-blocking work (git subprocess, chonkie) is wrapped in `to_thread` inside adapters. Pytest runs with `asyncio_mode = "auto"` — bare `async def test_*` works without decorators.
