...
答题路径会把有限正文窗口预算的四分之三留作可靠的语义情节保底。L1 的一致命中仍会增强并合并同一区域，余下四分之一则容纳精确的纯词法命中（尤其是标识符和日期）。这样，词法 span 与向量 span 不重合时，等权 RRF 不会仅因交错排序就悄悄丢掉 semantic top-k 的后半部分，同时也不会把答题路径变成纯向量检索。独立的 `rag` 接口仍保持普通的等权融合。
