...
| `AGENT_UNAVAILABLE_COOLDOWN_S` | `120` | harness 说的是**模型容量不足**（Codex：`Selected model is at capacity`）而不是订阅没额度时，同样的那段等待。容量几分钟内就会时有时无，所以它有自己的短等待：每次连续因容量被拒就翻倍，上限为 `AGENT_UNAVAILABLE_COOLDOWN_MAX_S`；第一轮真正跑起来的轮次会把它忘掉。在租户被晾起来之前，启动器对这种拒绝的重新拉起会拉开间隔，依次为 15 秒、30 秒、60 秒（带抖动），所以短暂的容量波动通常在一次启动之内就被消化掉。用量上限不受影响。不是引擎旋钮 |
