...
The framework enforces provenance for new/revised claims and newly broken dependants, system-assigned immutable anchors, read-only closed volumes, path ownership, and L0/L1 reachability ([architecture §5, §9](../architecture.md)). Unchanged historical provenance defects remain auditable but cannot ground new claims. Repeating these mechanics in the contract spends tokens and changes nothing — the gate does not read prose.
