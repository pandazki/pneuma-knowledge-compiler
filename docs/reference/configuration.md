...
| `QDRANT_COLLECTION` | `pneuma_knowledge_chunks` | one collection; its embedding dimension is fixed at creation — switching embedding models means a new collection name. A name that does not exist yet is created empty and WARNED about at startup: on a library that already holds vectors, a changed name is a silent loss of L2 and L3 semantic retrieval until something indexes into it |
...
| Setting | Default | Meaning |
|---|---|---|
| `LLM_MODEL` | `openrouter:openai/gpt-6-luna` | base model spec and fallback for all roles |
...
- **Every role without a door stays on a chat model.** A role that borrows `compile` — `challenge`, `brief`, `skill`, `groom` — skips a borrowed `agent:` spec and falls to `LLM_MODEL`, so one line (`models.compile: agent:codex`) never strands them. Rollover in particular: groom's single model call, the volume card, is a chat-model call even when compile runs on a coding agent, because groom has no draft door.
...
| `AGENT_RETRIES` | `3` | how many RAPID relaunches the unattended launcher may make INSIDE ONE JOB when the harness refused with a rate limit — and only that. Seconds apart, inside the launch this job already paid for; a model at capacity is spaced wider (15 s, 30 s, 60 s), because capacity usually returns within minutes. Every wait is logged. `0` means one attempt and no backoff. It is **not** how many times a job comes back: that is one rule with no knob (below). Not an engine knob |
...
| `AGENT_UNAVAILABLE_COOLDOWN_S` | `120` | the same wait for every refusal that is not a spent subscription: the MODEL is at capacity (Codex: `Selected model is at capacity`), the provider refused the connection (`401 Unauthorized`, `failed to connect to websocket`), or the harness died before its first turn (its own `thread.started` and nothing else). Capacity comes and goes within minutes, so this is its own short wait: it doubles per consecutive capacity refusal up to `AGENT_UNAVAILABLE_COOLDOWN_MAX_S` and is forgotten by the first round that actually runs. At the ceiling it keeps asking at the ceiling interval — this cooling is an optimisation (do not launch while we know the provider is down), never a verdict on the work, and nothing it does ever strikes a job out. Before the tenant cools at all, the launcher spaces its own relaunches for this refusal at 15 s, 30 s and 60 s (with jitter), so a brief dip is usually absorbed inside one launch. A usage limit is unaffected. Not an engine knob |
...
| *(no variable)* | `60, 300, 900, 3600, 14400, 86400` s | **how long a job waits before it is tried again, and how many times.** Not a setting: one named schedule in `job_retry.RETRY_BACKOFF_S`. Every failure that is not provably hopeless puts the job back to `queued` on its own row behind a `not_before`, with `waiting: <reason>; retry at <instant> (attempt n)` as its detail and the last ten failures in `payload.retry.history` — a harness that died, a round that accounted for nothing, a provider out of room or out of money, a network error, a gate that still refuses after the repair round, a library holding somebody's uncommitted changes. Nothing is struck out and no count fails a job. When the schedule runs out (the seventh failure) the row **pauses** instead: status `paused`, no `not_before`, `paused: <reason>; <n> attempts over <span>; resume with pkc jobs resume` — because what an empty account or a dirty tree is waiting for is a person, and `pkc jobs resume [--job \| --reason-like \| --all]` (or `POST /jobs/resume`) is that person, restarting the schedule from one minute with the history kept. The exceptions to all of it are enumerated in `job_retry.TERMINAL_FAILURES` (`unknown_kind`, `payload_invalid`, `source_gone`, `input_too_large`, `proposal_stale`) — those complete `ok=false` immediately and never enter the schedule, because the input has to change first. `GET /jobs/summary`, `pkc jobs` and `pkchome status` say what a library is waiting for and what is paused |
...
| `AGENT_KEEP_WORKDIR` | `false` | keep both of a round's temporary directories instead of deleting them: the launcher's per-round working directory (the system text, the task, the harness's last message) and the per-job config home the harness's own session transcript lives in. Debugging only: those files hold the library's material, so leaving them in `/tmp` is a decision an operator makes on purpose. Not an engine knob |
...
pricing: |
  ...
  openrouter:openai/gpt-6-sol  = 2/10/0.2/2.5 USD
```

These example GPT-6 rates are Standard rates for prompts up to 272K input tokens, checked on 2026-09-23; see the [provider pricing](https://developers.openai.com/api/docs/pricing) for long-context and processing-tier differences. A single rate declaration does not model those tier changes.

A spec is looked up exactly as it is routed (`openrouter:openai/gpt-6-luna`) and then by
...
| `LLM_MODEL_GROOM` | empty | model for the volume card a rollover writes; empty borrows the compile role, and a borrowed `agent:` spec falls to `LLM_MODEL` — the card is always a chat-model call |
...
| `RECALL_EVIDENCE_SELECTOR` | `model` | who composes the context under `RECALL_EVIDENCE_STRATEGY=select`: `model` is one structured recall-model call over the whole candidate pool; `scorer` hands that same pool to a calibrated evidence scorer — one usefulness score per candidate on a fixed 0–1 scale — and the keep/drop decision becomes `RECALL_SELECT_SCORE_FLOOR` in code. What follows the selection is identical either way (range validation, the ranked safety anchors, the per-face caps, provenance following); the scored selector picks no whole documents |
| `RECALL_EVIDENCE_SCORER` | (empty) | the scorer as `<provider>:<model>`, e.g. `typesafe:jev-1.13-20260917` (OpenRouter's decisions route). Required when the selector is `scorer`. Pin a dated version: the keep floor is fitted against one model's calibration |
| `RECALL_SELECT_SCORE_FLOOR` | `0.5` | keep a candidate the scorer places at or above this, on the port's fixed scale (`0.0` = a different subject, `1.0` = states the asked-for fact). Re-fit per deployment — it is a number about your material, not a constant of the framework. Read only when the selector is `scorer` |
| `RECALL_SELECTION_TIMEOUT_S` | `30` | the `select` stage's ceiling, model selector or scorer. Past it the lane answers out of its exact ranked heads and marks the answer `evidence_selection_degraded="timeout"` |
...
| `WORKER_SELFHEAL_S` | `60` | how often the worker applies its own startup self-heal while it runs (seconds). The sweep is the one a worker does before its first drain — `requeue_claimed_jobs` with its lease and lock rules: a live launch lease spared, a `pkc` command's lock respected, a dead launch's draft with work kept — minus the claims this process is running right now, which are nobody's orphans and which nothing on the row could tell apart (an index job carries neither a draft nor a lease). It exists because a claim used to outlive anything that went wrong between claiming a job and completing it: one library sat with 542 jobs pending for half an hour behind a derived-lane claim nobody put back, until a person restarted the engine. One log line when it requeues something, nothing when it does not. `0` turns it off, and the self-heal is again only what a process start does. It inherits the startup sweep's blast radius: two engines on one Postgres must set `WORKER_TENANTS`, as they already must for the sweep this repeats |
