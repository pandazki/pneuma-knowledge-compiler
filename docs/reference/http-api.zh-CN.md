...
两个读取接口的 `at` 都接受任意正本 ref（提交、标签、冻结快照）；不给则为 HEAD。无论哪种情况，返回体
自身的 `ref` 都指明读的是哪个提交——HEAD 在读取时被解析成具体提交，因为一次说不出自己读的是哪个
库的审读，事后与编译另一侧的那次审读无从分辨。只有当历史里一个提交都没有时它才为空。设计权威：
