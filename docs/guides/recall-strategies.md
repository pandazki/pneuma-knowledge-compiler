...
lane falls back to ranked claim/episode/window heads and marks the answer degraded; bounded
provenance reads still run, so this is not the complete `ranked` lane's context. It never invents
...
The number of added passages is bounded; their total characters are not. This can increase
context when the prior excerpts omitted a qualifier, and it still leaves interpretation to
the answer model. It does not add new knowledge to canonical.
