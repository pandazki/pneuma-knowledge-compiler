...
4. **A detected value is not a fact.** Anything auto-probed (timezone, language, …) is confirmed as "I detected X — change it?"; once confirmed, flip the matching `provenance` entry in `profile.yaml` to `profile`. Can't infer → ask; can't ask → leave empty and explain the default.
5. **The contract and the profile belong to the user, not to you.** Every draft is walked through with them, item by item, before it lands.

Open by explaining where the data goes; usable verbatim:

> First, where your data lives: your material, the databases, the compiled documents — all of it stays on this machine. I won't upload any of it anywhere, and nothing goes into anything that gets committed. The one exception is me — I run in the cloud, so what I read, and the fragments used when compiling or answering, are sent to the model provider you pick in `.env`. That's what it takes for the models to work, the same as any AI tool you use. Mask anything you don't want me to see, or stop me at any point.
...
**2.2 Get the key into `.env` — by THEIR hands, not yours.** The generated `.env` is complete except `OPENROUTER_API_KEY` (a key-blank `.env.example` sits beside it as the recovery copy). Have them open `.env` in their editor and paste the key themselves. Never take the key through your own commands, stdin, or the chat: everything that passes through an agent's tool calls lands verbatim in session transcripts, and a key in a transcript is a key on disk. You only verify the field is non-empty afterwards (`grep -c '^OPENROUTER_API_KEY=.' .env`). The defaults (`openrouter:openai/gpt-5.6-luna` + `text-embedding-3-small`) are a strong price/quality starting point; the compile model is the single quality lever, but **don't expand on that here** — it matters at step 6, when they start caring what deserves to be recorded.
...
3. **One paragraph for the four levels**: the raw material is stored verbatim (L0), full-text and semantic indexes are built over it (L1/L2), and on top sit a few "canonical" documents (L3) written by the compile model under the judgement in `contract.md`, every conclusion carrying citations back to the source. One-line summary: nothing of the original is lost; only what deserves long-term memory enters the canon.
4. **Say what the demo is**: everything in it (people, project, the example contract) is a synthetic placeholder, about to be replaced piece by piece with their own.
...
**Done when**: the detected values in `profile.yaml` are confirmed with `provenance` flipped to `profile`, and you can name the long-lived subjects in the material.

---

## 5. Step four: write `contract.md` together
...
- **Contract revision (judgement drift)**: when acceptance keeps flagging the same
  mis-filing, rewrite the relevant caliber in `contract.md` and bump `version:` — it
  governs future compiles only; nothing recorded is rewritten. Always cite to the user
  WHICH accrued material motivated the change.
...
**Tune the trigger to the data's rhythm** — the `.env` ships the knobs commented with
defaults (`EVOLVE_AUTO_TRIGGER=true`, fire after ~5 new docs AND ~30 new claims):
a slow personal library may want lower thresholds so structure keeps up; a daily bulk
feed may want them higher so proposals arrive weekly, not hourly; a fixed-schema
deployment can switch the trigger off entirely. Decide from the material's arrival
pattern you observed at step 4, and say why.
...
- Ask any time: `./app.py ask 'question'` (`--sources` prints the cited raw text too).
  Answers follow the project's answer-style preset (`PNEUMA_KNOWLEDGE_RECALL_ANSWER_STYLE`
  in `.env`, or per ask with `--style`): `concise` = the bare exact value, `conversational`
  = a natural chat reply (default), `detailed` = a self-contained written note. Pick for
  the consumer of the answers — a person chatting wants `conversational`; a script,
  grader, or benchmark judge that expects the exact short answer wants `concise`; written
  digests want `detailed`. Style never changes the truth discipline, only the shape.
...
- User data and personal information never land anywhere git might commit: the project directory lives outside repositories, or `git check-ignore` proves it ignored (the generated .gitignore covers only `.env` and `data/` inside the project).
- The API key goes only into the project's `.env`, typed there by the user's own hands: never into answers files (the generator refuses), never through your commands, stdin or chat — an agent's tool calls are recorded, so a key that touches them is a key persisted in a transcript.
...
- [ ] `contract.md` has no TODO and no demo content; `version` bumped;
- [ ] `profile.yaml` timezone/language confirmed by them, `provenance` flipped to `profile`;
