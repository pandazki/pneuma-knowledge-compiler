"""The AI-cue session policy, exercised with zero I/O and zero sleeps.
...
`CueSession` never reads a clock of its own; a state machine calling `time.monotonic()`
...
from pneuma_knowledge_core.recall.cue import label_turns
from pneuma_knowledge_service.context_stream.session import (
    SHOWN_MEMORY,
    CuePolicy,
    CueSession,
)
...
def cue(title: str, kind: str = "concept") -> dict:
...
def test_the_window_keeps_only_the_newest_turns():
    ...
    s = CueSession(CuePolicy(turn_window=3))
    for i in range(5):
...
def test_widening_the_window_at_runtime_keeps_what_is_already_held():
    s = CueSession(CuePolicy(turn_window=2))
    for i in range(3):
        s.add_turn(turn(f"t{i}"))
...
def test_the_first_evaluation_is_not_held_by_the_quiet_period():
    ...
    s = CueSession(CuePolicy(quiet_period=6.0))
...
def test_flush_skips_the_quiet_period_but_not_the_single_in_flight_rule():
    s = CueSession(CuePolicy(quiet_period=60.0))
    s.add_turn(turn("a"))
    first = s.begin(now=T0)
...
def test_a_stale_completion_cannot_free_the_slot_or_register_cards():
    ...
    s.add_turn(turn("a"))
    plan = s.begin(now=T0)
    assert s.complete(plan.seq + 7, [cue("Ghost")], now=T0) == []
...
def test_a_repeated_card_is_dropped_on_a_later_evaluation():
    ...
    same card surfacing two rounds later would otherwise pop onto the lens twice."""
    s = CueSession(CuePolicy(quiet_period=0.0))
    s.add_turn(turn("a"))
    first = s.begin(now=T0)
    assert [c["title"] for c in s.complete(first.seq, [cue("RAG")], now=T0)] == ["RAG"]

    s.add_turn(turn("b"))
    second = s.begin(now=T0)
    kept = s.complete(second.seq, [cue("RAG"), cue("HNSW")], now=T0)
...
def test_the_same_title_under_a_different_kind_is_not_a_repeat():
    ...
    s.add_turn(turn("a"))
    plan = s.begin(now=T0)
    kept = s.complete(plan.seq, [cue("X", "concept"), cue("X", "fact")], now=T0)
    assert len(kept) == 2
...
def test_a_plan_carries_already_shown_into_the_evaluation():
    ...
    s.complete(first.seq, [cue("RAG")], now=T0)
...
def test_the_client_is_the_dedup_authority():
    ...
    assert s.complete(plan.seq, [cue("从客户端恢复", "fact")], now=T0) == []
...
def test_the_shown_memory_is_bounded():
    s = CueSession(CuePolicy(quiet_period=0.0))
    s.configure(already_shown=[cue(f"c{i}") for i in range(SHOWN_MEMORY + 10)])
...
def test_the_confidence_dial_applies_to_the_next_evaluation():
    ...
    s = CueSession(CuePolicy(quiet_period=0.0))
    s.add_turn(turn("a"))
    assert s.begin(now=T0).min_confidence == CuePolicy().min_confidence
    s.complete(1, [], now=T0)
...
def test_an_unknown_focus_is_rejected_by_the_closed_vocabulary():
    ...
    bug in the caller, and quietly cueing under the wrong attention direction hides it."""
    with pytest.raises(ValueError, match="unknown cue focus"):
        CueSession(CuePolicy(focus="everyone"))  # type: ignore[arg-type]
    s = CueSession()
    with pytest.raises(ValueError, match="unknown cue focus"):
