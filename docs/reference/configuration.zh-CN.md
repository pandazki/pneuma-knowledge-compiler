...
| `QDRANT_COLLECTION` | `pneuma_knowledge_chunks` | 单 collection；向量维度建库时锁定——换嵌入模型要换 collection 名。名字不存在时会被创建为空，并在启动时告警：对已有向量的库，改名等于静默失去 L2/L3 语义检索，直到有东西索引进这个名字 |
...
| 配置 | 默认 | 含义 |
|---|---|---|
| `LLM_MODEL` | `openrouter:openai/gpt-6-luna` | 基础模型规格，也是所有角色的兜底 |
| `LLM_MODEL_COMPILE` / `_RECALL` / `_ANSWER` / `_GLANCE_PICK` / `_CALL` / `_DEEP` / `_SKILL` / `_EVOLVE` / `_LIVE_CONTEXT` / `_LIVE_DISCOVER` / `_LIVE_PICK` / `_CHALLENGE` / `_BRIEF` / `_GROOM` | 空 | 按角色覆盖；`answer` 只负责 fast 的最终答题，留空则借用 `recall` |
...
| `LLM_MODEL_CALL` | 空 | 语音通话的委托——夹在电话那头的语音和这座知识库之间的模型，每次提问做三件小事：从转写里写出问题、给 fast 车道选路、用 `spoken` 风格把答案说出来。这三件都发生在有人静静等着的时候，所以该用**快**模型，理由和概览挑选的一样；推理强度同样在代码里钉死为关闭，不在这里开旋钮。留空则借用 `recall`。语音本身是另一个模型，见下面的 `CALL_MODEL`。引擎键：`models.call` |
...
模型规格三种形态：`scripted:<路径>`（本地回放、零密钥——且硬覆盖所有角色，scripted 运行完全确定）；`openrouter:<模型>`（需要 `OPENROUTER_API_KEY`）；以及 `init_chat_model` 认识的任意 provider 前缀（如 `anthropic:claude-sonnet-5`、`openai:gpt-6-luna`）。角色回退只有一跳：`answer → recall`、`glance_pick → recall`、`call → recall`、`live_context → recall`、`live_discover → recall`、`live_pick → recall`、`evolve → compile`、`challenge → compile`、`brief → compile`、`skill → compile`、`groom → compile`，然后是 `LLM_MODEL`。

其中两个角色属于全量范围的实时上下文车道，它们之所以存在，是因为那条车道每一拍是两次小调用、而不是一次大调用（架构 §7）。`LLM_MODEL_LIVE_DISCOVER`（引擎键 `models.live_discover`）跑第①段——读待处理的对话，决定这一拍到底要不要检索——要的是**小型推理**模型：输出只有几十个 token，需要的是对一场对话的快速判断。`LLM_MODEL_LIVE_PICK`（引擎键 `models.live_pick`）跑第③段——在已经装配好的候选卡片里选一张或一张都不选、写一句短引言、裁剪引用、打分——要的是**又弱又快**的模型，因为这里没有什么要推理的：证据就摆在面前，而且它一个字都不许改写。生成出来的引擎分别写的是 `openrouter:openai/gpt-6-sol` 与 `openrouter:openai/gpt-6-luna`；两者留空都借用 `recall`，于是已有部署原样继续工作。它们的推理强度由**框架钉死**（发现为 `low`，挑选关闭），并且刻意不做成旋钮：能被部署调高的强度会改变这条车道每一拍的成本，而便宜正是「先花一次调用、再决定要不要检索」这件事的全部理由。`LLM_MODEL_LIVE_CONTEXT` 仍然负责简报范围的那一轮与卡片展开，两者各一次调用，均未改变。脚手架默认让 `recall` 使用 Luna，`answer` 和 `answer_reasoning_effort` 留空：fast 最终答题借用 `recall`，保持 provider 的默认推理强度。单独指定答题模型或推理强度是部署方的可选配置。
...
- **没有门的角色一律停在聊天模型上。** 借用 `compile` 的角色——`challenge`、`brief`、`skill`、`groom`——会跳过借来的 `agent:` 规格、落到 `LLM_MODEL`，所以一行（`models.compile: agent:codex`）不会把它们撂在半路。轮转尤其如此：groom 只有一次模型调用（分卷卡片），即使编译跑在编码代理上，它也仍是一次聊天模型调用——groom 没有草稿门。
...
| `AGENT_RETRIES` | `3` | 当 harness 以**限流**拒绝一轮时，无人值守启动器在**同一个作业内**最多可以快速重新拉起几次——仅限这一种。间隔以秒计，发生在这个作业已经付过的那次启动里；模型容量不足时间隔拉得更开（15 秒、30 秒、60 秒），因为容量通常几分钟内就会回来。每一次等待都写日志。`0` 表示只试一次、不退避。它**不是**作业能回来几次：那是一条没有旋钮的规则（见下）。不是引擎旋钮 |
...
| `AGENT_EPISODES_WINDOW_CHARS` | `400000` | 编码代理的 **episodes** 任务最多携带一个来源的多少内容。episodes 轮的任务就是它要判断的材料，所以更长的来源不截断，而是按由整块组成的连续窗口分轮判断——每个窗口一个 episodes 作业、一轮，每份判断都是独立的留存记录，全部窗口都记录后才写入该来源的 L2。按任务中渲染后的块与章节条目计量，而不是裸文本。默认值与个人版 `max_part_chars` 同一量级，在 Codex 1,048,576 字符的输入上限之下为系统文本与技能留出空间。`0` = 单一窗口、不设上限。不是引擎旋钮 |
...
| `AGENT_UNAVAILABLE_COOLDOWN_S` | `120` | 凡不是订阅没额度的拒绝，都用这段等待：**模型容量不足**（Codex：`Selected model is at capacity`）、提供方拒绝连接（`401 Unauthorized`、`failed to connect to websocket`），以及 harness 没跑到第一个回合就死了（只有它自己的 `thread.started`，再无别的）。容量几分钟内就会时有时无，所以它有自己的短等待：每次连续因容量被拒就翻倍，上限为 `AGENT_UNAVAILABLE_COOLDOWN_MAX_S`；第一轮真正跑起来的轮次会把它忘掉。翻倍到顶之后就按上限的间隔继续等：这份晾置是一项**优化**（明知服务商不可用时就不要拉起），从来不是对工作本身的判决，它做的任何事都不会把一个作业划掉。在租户被晾起来之前，启动器对这种拒绝的重新拉起会拉开间隔，依次为 15 秒、30 秒、60 秒（带抖动），所以短暂的容量波动通常在一次启动之内就被消化掉。用量上限不受影响。不是引擎旋钮 |
| `AGENT_UNAVAILABLE_COOLDOWN_MAX_S` | `900` | 容量翻倍停在哪里：十五分钟，而不是用量上限的六小时。不是引擎旋钮 |
| *（无变量）* | `60, 300, 900, 3600, 14400, 86400` 秒 | **一个作业等多久才被再试一次、以及一共试几次。** 它不是配置项：`job_retry.RETRY_BACKOFF_S` 里一张写死的表。凡不是「明确重试无法成功」的失败，作业都会回到自己那一行的 `queued`，带上 `not_before`，detail 写作 `waiting: <原因>; retry at <时刻> (attempt n)`，最近十次失败留在 `payload.retry.history` 里——harness 死了、一轮什么都没交代、服务商没额度或没钱、网络出错、修复轮之后闸门仍然拒绝、库里还压着别人未提交的改动，都是如此。没有任何东西被划掉，也没有任何计数会把作业判失败。等待表走完之后（第七次失败），这一行改为**暂停**：状态 `paused`，不带 `not_before`，detail 写作 `paused: <原因>; <n> attempts over <时长>; resume with pkc jobs resume`——因为一个空账户、一棵脏工作树等的是人，而 `pkc jobs resume [--job \| --reason-like \| --all]`（或 `POST /jobs/resume`）就是那个人：退避表从一分钟重新开始，历史保留。这一切的例外在 `job_retry.TERMINAL_FAILURES` 里逐条列明（`unknown_kind`、`payload_invalid`、`source_gone`、`input_too_large`、`proposal_stale`）——这些直接以 `ok=false` 结束，根本不进入等待表，因为必须先改输入。`GET /jobs/summary`、`pkc jobs` 与 `pkchome status` 会说明一个库在等什么、有什么被暂停了 |
...
| `AGENT_KEEP_WORKDIR` | `false` | 把一轮的两个临时目录都保留下来，而不是删掉：启动器为每一轮建的工作目录（system 文本、任务、harness 的最后一条消息），以及 harness 自己那份会话记录所在的 per-job config home。仅供调试：这些文件里装着知识库的材料，把它们留在 `/tmp` 应当是运维者刻意做的决定。不是引擎旋钮 |
...
`native` 是一次明确断言：选中的模型和实际路由 provider 能接收 LangChain 图片 content block；不兼容就失败，不会悄悄把图片压成文本。`caption` 要求 importer 提供带标签的 `caption`/`ocr` 表示，也绝不声称编译模型看过原图。`auto` 会把直连 OpenAI 和 OpenRouter 上的 GPT-6 Luna/Sol 和 GPT-5.6 全系识别为原生图片模型，即使网关没有附 LangChain model profile；其他能力未知的 profile 保持保守的 `auto → caption`。
...
这里的 GPT-6 示例费率为不超过 272K 输入 token 的 Standard 费率，核对于 2026-09-23；长上下文与处理档位的差异见[供应商定价](https://developers.openai.com/api/docs/pricing)。单份费率声明不表达这些档位变化。

查价先按实际路由的完整规格（`openrouter:openai/gpt-6-luna`），再按去掉 provider 前缀后的裸模型
...
| `LLM_MODEL_GROOM` | 空 | 轮转写分卷卡片用的模型；留空借用编译角色，借来的 `agent:` 规格则落到 `LLM_MODEL`——这张卡片永远是一次聊天模型调用 |
...
抵达同一批车道的第四条路：一个全双工语音模型主持通话，它对知识库一无所知；知识库知道的一切，
都经由本进程运行的委托交给它——从转写里把问题写出来、走 fast 车道、用 `spoken` 风格作答
（[设计](../design/voice-call.zh-CN.md)）。委托用的模型是上面的 `LLM_MODEL_CALL`；下面这四项
是通话本身。

| 配置 | 默认 | 含义 |
|---|---|---|
| `CALL_MODEL` | `gpt-live-1` | 主持通话的语音模型。用 `OPENAI_API_KEY` 连接，而这个密钥从不离开本进程：交给浏览器的是一份 SDP 应答，不是凭据。没有密钥时，本部署把通话报为未配置，而不是把它摆出来。引擎键：`recall.call_model` |
| `CALL_VOICE` | `marin` | 该模型用哪把嗓子说话。名字取自语音供应商的词表，创建会话时原样传过去。引擎键：`recall.call_voice` |
| `CALL_IDLE_SECONDS` | `180` | 一通电话里，知识主体多少秒没出声，引擎就把它挂掉。引擎键：`recall.call_idle_seconds` |
| `CALL_MAX_SECONDS` | `1800` | 一通电话最多跑多少秒，聊得再热闹也到此为止。引擎键：`recall.call_max_seconds` |

后两项之所以存在，是因为语音会话只要开着就按分钟计费，有没有人说话都一样：没人在说的电话在这里
被结束，而不是留着跑；任何一通电话都活不过第二道上限。`0` 可分别关掉。
...
滞后于它那次咨询一个队列排空的时间（按用户、FIFO、每道同时一个作业在飞，而投影在派生道上：闲置的库是几秒，
也不再排在编译之后——编译走的是另一条道，architecture.md §5）。`scripts/ops/rebuild_derived.py` 通过入队一个 `recall_rebuild` 作业并把它排掉来
...
| `OPENAI_API_KEY` | [语音通话](#语音通话)因它而存在：语音模型是 OpenAI 的。只有本进程会用到它——浏览器收到的是一份 SDP 应答，从不是密钥。不设置时 `/v1/call/*` 报通话未配置，其余一切照旧。任何 `openai:<模型>` 规格也用这个密钥 |
