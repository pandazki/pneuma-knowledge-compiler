 ...
 * captures screenshots under this example, records console output and mobile
 * horizontal-overflow checks. Requires:
 *   - seeded demo tenant (uv run python -m examples.opc seed)
 ...
 * V2:
 *   E2E_USER=<tenant> E2E_SCOPE=sources-v2 node examples/opc/e2e/screenshots.mjs
 */
...
const INGEST_MD = `# 公开预览演示笔记（synthetic）

## 背景

这是一段用于 UI 验收的合成材料，记录当前项目公开预览前的准备事项，与真实业务无关。
...
async function j02Ingest() {
  ...
  await panel.getByLabel("标题", { exact: true }).fill("公开预览演示笔记");
...
async function j05Recall() {
  ...
  await page.getByLabel("检索查询").fill("当前项目发布门禁");
...
async function j06Ask() {
  const { context, page } = await newPage({ theme: "light" });
  await go(page, "/ask");
  await page.getByLabel(/scope\.query/).fill("当前项目发布门禁");
  ...
  await page.getByLabel("提问").fill("当前项目发布前必须满足哪些门禁？");
  await page.getByRole("button", { name: "提问", exact: true }).click();
  await page.getByText(/首版范围是来源导入/).waitFor({ timeout: 30000 });
...
async function j07ContextSuggestion() {
  ...
  await speakers.nth(0).fill("本人");
  await texts.nth(0).fill("我在整理当前项目的公开发布清单，同步一下发布进度。");
