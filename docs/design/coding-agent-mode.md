...
| `pkc canonical ls` (`--include-archived`) / `read <path> [<path> …]` / `history <path>` | pages, a page, a claim chain; each read shows `last changed` (path commit), ledger claims as counted by outline, overview blocks, repository-wide supersession and successor addresses, `latest cited source`, and pending + failed compile jobs for this tenant. The source index lists cited blocks with speakers and their own recorded days; missing authorship is `unknown`. Source dates use occurrence days, else `imported …`; `read` and `history` are unconditional |
| `pkc source ls` (`--include-archived`) / `show <id>` / `fetch <id> ¶a-b [<id> ¶c-d …]` | L0: sources, structure, verbatim spans; each fetched span has its full source id, per-block speaker (explicit `unknown` when absent), recorded block day and source occurrence day or labelled import day. JSON is a list, wrapped as `items` with `paging` when paged; single-source multi-span reads still work; `show` and `fetch` are unconditional |
...
| `pkc search <q>` (`--lexical` / `--semantic` / fused; `--include-archived`) | L1 / L2 in the requested scope; ranked hits with per-block speakers and days. Lexical/fused headers show estimated per-term and all-terms **indexed blocks**, state single-block matching, possible adjacent-block joins and index-queue lag behind L0, and count pending index jobs for this tenant. Quoted phrases are one term; semantic has no lexical counts; fused reports its lexical total separately |
| `pkc recall <q> --evidence` (`--include-archived`) / `pkc recall --evidence --handoff <id> --page N` | fast evidence without the answer call: header with tallies, handoff on line 3 and exact next-page command; claims, verbatim windows, episode summaries (derived), then map. Sections with relevance scores are rank-ordered; otherwise labelled as in the lane’s order. The handle-to-source index lists cited blocks, speakers and days. Prose is retained with the handoff, including silent calls: `--handoff` pages it without retrieval or a second handoff; `--page` alone starts a new retrieval. JSON preserves the whole lane `content`, with `tally` and `sources` |
...
Read output pages prose by characters and JSON by whole list items. A top-level list is
wrapped as `items` when paged; in an object with several lists, the largest serialized list
is paged and named in `paging.list`. `--all-pages` returns the whole payload. Recall evidence
JSON stays whole. Retained recall pages reuse the original page size and repeat the header;
they remain available until the handoff is answered or expires.
