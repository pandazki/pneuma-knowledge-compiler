...
Modality never erases provenance. An original image, audio recording or file that the compile model received natively and that remains retrievable through the claim's cited L0 address may support a direct observation. A caption, transcript, OCR result or search query produced by a model, dataset or adapter is a derived observation: attribute it to that producer and preserve its uncertainty. A URL alone is not visual evidence. Never write "the image shows" when the model only received a caption; write "the supplied caption describes" instead.
