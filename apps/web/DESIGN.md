...
| Canon | 08 / 09 / 10 | `library` / `lens` / `history` | canonical documents, the structure lens, versions |
...
- **lens** — the *structure lens*: the whole library read from outside, by a derived, model-free reader in the service (docs/design/structure-lens.md). The console computes none of it, which is the point — the number here is the number a Steward running `pkc lens` sees. Two tabs. **Reading** opens on the score — the share of subjects no open finding names — with the one line that says it is not a grade, then **the three things to do first** — the first finding of each of the three highest-ranked *lenses*, one per lens, so three islands cannot crowd out a duplicated subject and a malformed page. Each is a sentence saying what it costs (the report carries every sentence already rendered in both packs, so the console keeps no copy of the catalog's wording), the action under it, the actor as a word, its pages clicking into the document (past twelve they fold behind a `+N more` that expands in place, because one finding can name a hundred subjects), and its evidence; below that the same findings again by level — principle / drift / shape — folded behind their counts. **Compare** reads the report at two refs and subtracts: the score and base counts, then the findings resolved / new / still open, matched by key so "still open" means the same fault on the same page; new internal links, each with the sentence that made it, are a second read behind a button, because that sentence lives in the canonical projection and two projections are expensive. This is a READING face and nothing else: nothing on it hands a finding to anybody (§9 of the design reserves that). Its predecessor `#/graph` redirects here, and old `#/graph/node/<id>` links resolve to the document (or source) the node stood for.
