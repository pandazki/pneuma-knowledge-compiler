...
| `recall.fast` | `recall.fast`, `recall.fast.plan`, `recall.fast.select`, `recall.fast.evidence_select` | `POST /v1/users/{user_id}/recall` with `mode=fast` | one answer call (`recall.fast`), plus one span per model step that ran: `plan` when `RECALL_PLAN_QUERIES` is above zero, `select` when `ranked` picks whole documents off the glance (on the `glance_pick` role, which the span's own generation names, so a pick that misses its 8-second ceiling is attributable to a model rather than to the pass), `evidence_select` when `evidence_strategy: select` composes the evidence |
