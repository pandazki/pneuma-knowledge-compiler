...
export default function Search({ state, busy, perform, query, composing, locale }: {
  state: Snapshot; busy: boolean; perform: Perform; query: string; composing: boolean; locale: Locale;
}) {
  ...
  if (!current) return <p className="muted" role="status">{t(locale, 'searching', { name: library.name })}</p>;
