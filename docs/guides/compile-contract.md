...
The framework already enforces a set of rules mechanically: provenance on new or revised ledger claims (a source citation or reference to another existing ledger anchor), system-assigned immutable anchors, read-only closed volumes, path ownership, L0/L1 reachability ([architecture §5, §9](../architecture.md)). Repeating any of them in the contract spends tokens and changes nothing — the gate does not read prose.
