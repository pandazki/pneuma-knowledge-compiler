...
Hash routing is the deep-link contract (`lib/hash.ts`): every view name plus selection encoding. Navigation is a book's table of contents — a contents rail on desktop, a Drawer on mobile. The § numbers, order and grouping live in `components/TocNav.tsx` (`TOC`), the words in `i18n/nav.ts`: numbering is structure, labels are copy.
...
**The model lanes are a bench, and a key opens it.** In the personal edition the library holds no model — every judgement is the agent's — so Recall, Ask and Live Context are quality-testing tools over the API's model lanes rather than the way anybody retrieves anything day to day; retrieval for daily use is an agent session, which is the `steward` view. Those lanes need an API key, and `lib/modelLanes.ts` is the one declaration of both halves: the three views, and the single condition that closes them (a home answered, and its current library reports `key: false`). The gate is the ROUTER's (`App.tsx`), so a deep link lands on the notice by the same road a click on the rail does and the three view files never learn the concept exists; the notice itself (`views/_shared/ModelLaneNotice.tsx`) keeps the page's own title and says three things — what this page is, why it cannot run (with `pkchome credentials set OPENROUTER_API_KEY`), and where retrieval went, offering the Steward only to a lens that may see it. The contents rail keeps all three entries and MARKS them with a small `key` badge: a credential must not edit the table of contents. Without a home — every project deployment — nothing is locked and every page renders byte for byte as it did before this existed.
...
- **library** — the document tree on the left; on the right the selected document as a proof: serif body, mono claim anchors, footnote citations, and flags as marginalia. A selected claim deep-links to `#/library/claim/…`. The neighbourhood card (§4.2) is where a thread gets followed. The archive is folded out of the contents into a collapsed section at the foot of the rail (`lib/archive.ts`, the `archive/` prefix and nothing else) — an archived page still opens, and says so on a badge; the owner's header action opens the proposal dialog (`views/archive/`), and the page header's `Archive` opens the inventory of what is in there now.
- **lens** — the *structure lens*: the whole library read from outside, by a derived, model-free reader in the service (docs/design/structure-lens.md §4). The console computes none of it, which is the point — the reading here is the reading a Steward running `pkc lens` gets. Six sections, one per dimension: walkability, shape, knowledge vs log, liveness, type and structure, demand and supply. Each opens on its **band** as one word (open / thin / broken, even / leaning / collapsing, …) — neutral ink, because a band is a reading and not a state — then the **statement** at reading size, a short **metrics** table carrying the movement since the previous reading (now / before / Δ, an absent number as an em dash), the **direction** (the lever: a contract clause, an evolve, a groom, a review round — never a page), and the evidence. A picker chooses what the previous reading is read at; left alone it sends nothing and the service reads HEAD's parent itself, so the console can never disagree with `pkc lens` about which commit that is. No score, no findings and no list of pages: a finding belongs to the lowest tier that can see it, and the check has its own face. The statement and the direction arrive already rendered in both packs and the console keeps no copy of them; the dimension titles, the band words and the metric labels are the console's own chrome, looked up by a runtime string so an unknown one renders under its own name. Its predecessor `#/graph` redirects here, and old `#/graph/node/<id>` links resolve to the document (or source) the node stood for.
- **review** — the *check* (docs/design/structure-lens.md §3): the middle tier, what an insider standing at the page can see against the contract and repair in a round of its own — a hub leaving its own children unreachable, a subject named twenty times and never linked, a title that exists twice, plus the legacy instances of faults the gate now refuses at the write. Grouped by PAGE, because that is how the work is done: one document opened, everything about it fixed, rather than a flat list that sends a Steward back to the same file four times. Within a page the report's own order stands (legacy before judgement, a hook fault being unambiguous), and each row is the finding's sentence, its id in mono, its kind, the repairing verb, the other pages it touches and its evidence. The header counts the findings by kind beside the ref and the read time. The one act on the page is the review ROUND: a canonical-lane job the Owner enqueues, answering with its job id and queue state and a way into `process`. Nothing else hands a finding to anybody.
...
- **home** (`#/home`, listed only where a home answered) — the machine, not the library: Docker and the four services it runs once, then every library on it as a hairline-separated block (engine up / port / pid / uptime, queue pending and failed, last compile, key, skill freshness, the five-step cold start, last used), closing on the home path and version. Read-only; the actions are `pkchome` verbs behind `/home/actions/*`, and a page that offered buttons over an endpoint that does not exist yet would not be honest.
