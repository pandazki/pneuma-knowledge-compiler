...
- 即使有 LLM 参与，重建也是字节确定的。语义分块（默认的 `semantic` 策略，边界检测哲学受 [nemori](https://github.com/nemori-ai/nemori) 启发）用 LLM 检测主题/情节边界，但模型只返回块索引——chunk 文本永远是原文的逐字切片，出处寻址与机械切块完全一致；首次检测出的边界连同内容摘要、模型来历与产出它的输出契约一并记进清单，重建时回放而不是重算。一个情节若长于部署的 `CHUNK_SIZE`，会先按句子再切后才做 embedding，避免语义连贯的边界变成一个被稀释的数千字检索单元。keyless/scripted 模型下，边界检测自动回落机械句切。每个逐字 chunk 的向量输入还会携带来源标题、已知发生日期，以及其覆盖块上带明确标签的 caption/OCR；Qdrant 中保存的仍是逐字 chunk 及其精确区间。因此时间与图片语义都可以被检索，而元数据或派生表示不会冒充原文，也不会产生第二套引用方式。
