...
DEFAULTS: dict[str, str] = {
    ...
    "recall.retrieval.receipts_header": "# lookup scopes ({count})",
    "recall.retrieval.component": "Structured component lookup; the displayed arguments apply to this lookup only",
    "recall.retrieval.question_context": "as_of: {as_of}\nsubject_timezone: {zone}\nquestion: {question}",
    "recall.retrieval.method": "method: {method}",
    "recall.retrieval.transferred": "{count} lookup items are shown in other evidence sections; this block retains the lookup scope.",
    "recall.retrieval.origin": "route: {route}\nmethod: {method}\nlookup scope: {arguments}",
    "recall.retrieval.group": "<retrieval>\n{origins}\n{evidence}\n</retrieval>",
    "recall.retrieval.unknown": "unknown",
    "recall.retrieval.time_unknown": "source occurrence / cited block clocks: unknown (no resolved metadata)",
    "recall.temporal.empty": "This lookup found no records with verifiable source times in {since}–{until} ({zone}). It cannot establish facts for that period.",
    "recall.temporal.unresolved": "The lookup could not establish a supported source-date interval from the question. Its requested date range remains unresolved.",
    "recall.temporal.unavailable": "Source-time validation was unavailable for this lookup. No facts were released because their applicability to the requested time scope could not be checked.",
    "recall.retrieval.source_time": "{citation} source occurred_on={occurred_on}; cited block clocks (UTC)={first}..{last} ({known}/{total} blocks dated)",
    "recall.retrieval.claims": "L3 lexical/semantic relevance rankings fused by RRF; occurrence time is not a search filter",
    "recall.retrieval.windows": "L1 lexical and L2 raw/episode semantic rankings fused by RRF; occurrence time is not a search filter",
    "recall.retrieval.provenance": "Exact L0 span followed from a selected citation; no independent time filter",
    "recall.retrieval.map": "Library layout from canonical; no question-specific relevance or time filter",
    "recall.retrieval.documents": "Whole canonical pages selected for relevance; no time filter within a page",
    "recall.retrieval.timelines": "Sibling canonical claims expanded from hit subjects in document order; no time filter",
    ...
    "steward.cli.jobs_requeue_kind": "only jobs of this kind",
    ...
    "steward.cli.jobs_resume": "start paused jobs again with a fresh retry schedule",
    "steward.cli.jobs_resume_description": "A job that used up the retry schedule is PAUSED: it stopped asking and is waiting for a person, because whatever it was waiting for — an account with no money on it, a harness nobody logged in, a working tree somebody left dirty — does not change by being asked a seventh time. This is the person saying they have done that thing. The selected rows go back to `queued` with `payload.retry.attempts` reset to 0, so the next failure waits a minute rather than the day the exhausted schedule ended on; the failure history is kept. At least one selector is required. Exit codes: 0 ok, 1 nothing matched, 2 refused.",
    "steward.cli.jobs_resume_job": "only this paused job, by id",
    "steward.cli.jobs_resume_reason_like": "only paused jobs whose reason contains this text",
    "steward.cli.jobs_resume_all": "every paused job of this library",
