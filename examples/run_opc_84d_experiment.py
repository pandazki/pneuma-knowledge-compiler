...
``--mode scripted`` is a fast, keyless ingestion/index baseline over real
...
DEFAULT_REPORT_DIR = Path("docs/experiments/results")
...
def _build_dataset():
    """Load only the current, globally validated accepted v2 corpus."""

    return build_accepted_opc_84d_v2_dataset(project_root=PROJECT_ROOT)
...
def _settings(mode: str) -> Settings:
    ...
    for role in ("compile", "recall", "deep", "live_context", "evolve"):
        ...
        if not model.startswith("openrouter:"):
            issues.append(f"{role} is not OpenRouter: {model}")
    if not settings.embedding_model.startswith("openrouter:"):
        issues.append(
            f"embedding model is not OpenRouter: {settings.embedding_model}"
        )
...
async def _profile(ctx, user_id: UserId) -> None:
    ...
    await ctx.store.upsert_user_profile(user_id, payload)
...
async def run(
    *,
    ...
    # Accepted v2 sources carry evidence links rather than the old synthetic truth
    # manifest. In scripted mode that intentionally yields no fabricated claims.
    truth_values: tuple[str, ...] = ()
            ...
            model = (
                ...
                if mode == "real"
                    ...
                    turns=await _scripted_turns(
                        ctx, user_id, source_ids, truth_values
                    )
                )
            )
            processed = await drain_user(
                ctx, model, load_builtin_skill(), user_id
            )
...
def main() -> int:
    ...
    parser.add_argument(
        "--report",
        type=Path,
        help="report path; defaults to a user-scoped file under docs/experiments/results",
    )
