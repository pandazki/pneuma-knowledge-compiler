...
- 即使有 LLM 参与，重建也是字节确定的。语义分块（默认 `semantic` 策略，边界哲学受 [nemori](https://github.com/nemori-ai/nemori) 启发）在同一份结构化 LLM 响应里，为每个主题/episode 返回便于搜索的标题、第三人称事实描述和块坐标。描述保留覆盖块支持的具体参与者、时间、地点、事件、决定、情绪、原因、计划与结果；来源发生日期已知时才换算相对时间，未知时不猜。这份生成内容只是派生检索文本，不是来源：chunk 文本仍是与机械切块同样寻址的逐字切片。v3 manifest 记录坐标与标题/描述，重建时直接回放。只有边界的旧 manifest 会被一次性补描述，但区间必须逐一与旧记录相等，任何试图改边界的输出都被机械拒绝。超过 `CHUNK_SIZE` 的 episode 会再按句切；scripted/keyless 模型仍回落机械句切。L2 保存两套独立排序的表示：raw 向量只嵌入来源上下文、带标签 caption/OCR 与逐字 chunk；每个 episode 另有一个只嵌入来源上下文和标题/描述的向量。每个 point 都解析到对应的逐字 L0 区间，生成文本从不作为证据存储或展示。检索用普通 RRF 融合词法、raw 向量和 episode 向量，再抑制排名较低的重叠区间；胜出区间不扩宽，重复分数也不累加。因此更好的检索语义既不会稀释 raw 向量，也不会创造第二份权威或引用空间。
