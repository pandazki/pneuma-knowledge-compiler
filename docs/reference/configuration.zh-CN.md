...
**Episode 表示。** 同一份结构化响应里，每个语义段按「语义在前」的顺序返回 `title`、`description`，再返回 `start`/`end` 坐标（`off` 的 `end` 由下一个 start 推导，因此省略）。描述只跟随来源：具体人物、时间、地点、事件、决定、情绪、原因、计划与结果；只在来源发生日期可作锚点时换算相对日期。标题/描述、来源日期/标题、caption/OCR 与逐字 chunk 一起做 embedding；存储与答题证据只保留逐字 chunk。新 v3 manifest 无模型回放坐标与表示；只有边界的旧 manifest 会执行一次固定区间补描述，返回坐标必须逐一完全相等。
...
重叠会让一个块出现在两个 L2 chunk 里。这份重复完全在派生层：L0 不受影响，两个 chunk 通过同一套寻址方案指向同一批源块，而检索侧本来就会把重叠窗口合并成一段，所以经由两个 chunk 命中的转折块只会被读一次。chunk manifest 记录了产出这批区间的模式，因此翻转这个旋钮再跑 `rebuild_derived` 会真正重切，而不是回放旧布局。旧记录的坐标始终原样保留；一次性 v3 迁移只补它们缺失的派生描述。
