...
| `AGENT_RETRIES` | `3` | 当 harness 以**限流**拒绝一轮时，无人值守启动器最多可以重新拉起几次——仅限这一种。其他任何拒绝都只上报不重试（再试一次还是会被拒），超时同样不重试，因为墙钟本身就是「这一轮结束了」的声明。等待按指数增长并带抖动，受启动器自身的上限约束；模型容量不足时间隔拉得更开（15 秒、30 秒、60 秒），因为容量通常几分钟内就会回来。每一次等待都写日志。`0` 表示只试一次、不退避。不是引擎旋钮 |
