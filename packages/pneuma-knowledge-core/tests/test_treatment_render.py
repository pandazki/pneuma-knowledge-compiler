...
def test_each_treatment_emits_its_fixed_instruction():
    ...
    assert _TREATMENT_INSTRUCTIONS["full"] in task
    assert _TREATMENT_INSTRUCTIONS["distill"] in task
...
def test_missing_or_unknown_treatment_defaults_to_full():
    ...
    assert _TREATMENT_INSTRUCTIONS["full"] in default_task
    assert _TREATMENT_INSTRUCTIONS["full"] in unknown_task
