...
把你领域里的原始材料——会议、文档、聊天、邮件——编译成可演进、带明确出处的知识库。
...
编写的账本主张必须沿出处链抵达来源或已准入的机械记录，未改动的主张也不例外，没有来源的循环不算依据。框架检查引用语法、来源区间，以及所有已有总览中的每条声明引用。机械归档记录和卷目录保留各自狭窄的准入规则。这些检查建立可追溯的地址，不证明解释忠实；语义保真仍由编译契约、模型和审阅承担。
