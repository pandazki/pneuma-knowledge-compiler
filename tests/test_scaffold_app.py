...
@pytest.mark.parametrize("committed_with_error", [False, True])
def test_earlier_success_cannot_resolve_a_later_failed_compile(committed_with_error):
    old = _compile_job(not committed_with_error, "2026-01-01T10:00:00+00:00")
    if committed_with_error:
        old["snapshot_ref"] = "already-committed"
...
def test_a_committed_job_also_covers_its_sources_for_the_other_failures():
    ...
    jobs = [
        _job(job_id="old", detail="rejected", completed_at="2026-01-01T10:00:00Z"),
