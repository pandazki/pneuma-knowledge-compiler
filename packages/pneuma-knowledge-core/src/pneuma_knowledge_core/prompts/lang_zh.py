...
_ZH: dict[str, str] = {
    ...
    "steward.cli.jobs_requeue_kind": "只要这一类作业",
    ...
    "steward.cli.jobs_resume": "让暂停的作业重新开始，并重置退避表",
    "steward.cli.jobs_resume_description": "用完整张重试表的作业会进入**暂停**：它不再发问，而是在等一个人——因为它等的那件事（没钱的账户、没人登录的 harness、别人留下的脏工作树）不会因为被问第七次而改变。这条命令就是那个人说「我已经把那件事做了」。选中的行回到 `queued`，`payload.retry.attempts` 归零，于是下一次失败等一分钟，而不是用完的表末尾那一天；失败历史保留。至少要给一个选择器。退出码：0 成功，1 没有匹配，2 拒绝。",
    "steward.cli.jobs_resume_job": "只选这一个暂停的作业（按 id）",
    "steward.cli.jobs_resume_reason_like": "只选原因里含这段文字的暂停作业",
    "steward.cli.jobs_resume_all": "这个知识库里所有暂停的作业",
    ...
    "steward.read.showing_semantic": "显示：{showing} 条语义结果（按排名；--limit N 可查看更多）",
    "steward.cli.outline": (
...
Owner 陈述与 evolve 流程。两种姿态都不会把回答变成 canonical 知识。在 agent 执行器下由 agent
完成这些阅读；fast/deep/briefing API 通道是有密钥的控制台用来测试质量的工具。
""",
