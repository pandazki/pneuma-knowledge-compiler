...
Steward 正文与问答页共用编号来源脚注，点击可打开所引原文区间。原始会话文本不变，这种渲染不证明主张正确。命令行保留原命令及可展开的输出；小状态图标在悬停、键盘聚焦或点按时显示退出码与耗时，每轮用量也通过图标展开带标签的指标。错误仍在对话中明确显示。
...
编译读 L0，绝不等待 episodes；技能中的阅读顺序不增加队列依赖。episodes 作业沿用其 index 作业在队列中的
位置，因此通常先于该来源的编译被领取（§9）——这是次序，不是等待。
...
**窗口。** episodes 轮没有第二种途径读取它要判断的材料——任务本身就是材料——所以任务会超过
`AGENT_EPISODES_WINDOW_CHARS`（默认 400,000 字符，按渲染后的块与章节条目计量）的来源，
按由整块组成的连续窗口分轮判断，绝不在块内切开；单个超出上限的块自成一个窗口。worker 领取
这种来源的无窗口 episodes 作业时不启动任何一轮：它为每个窗口入队一个作业，payload 为
`window: {start, end}`（闭区间块跨度，与所有跨度同一套寻址，I4），全部沿用原作业在队列中的
位置，从而都排在该来源的编译之前；原作业以 `ok` 完成，detail 为 `episodes: split into N windows`。
交互式 `open` 做同样的事。这也迁移了窗口出现之前已入队的作业。窗口的一轮只看到自己的块和
伸入其中的章节，使用来源自身的编号；门拒绝落在窗口外的端点，`no episode` 余项只列本窗口的块
——空隙依然合法，`[]` 依然表示这里没有值得进入 L2 的内容。每个窗口的判断是
`chunk_manifest_windows` 中一条独立的留存记录，键为 `(租户, 来源, window_start)`，重放键与
整源清单相同；窗口绑定整个来源的内容摘要，因此任何一处编辑都会让所有窗口失效。来源的判断
是覆盖它的已记录窗口按块序拼接的结果。在每一段都被覆盖之前，来源没有 L2 分块，与未判断的
来源完全一样；最后一个窗口的 `finish` 一次性替换写入它的 L2；重建重放这些窗口。
`chunk_manifests` 不变：整源清单照旧有效，匹配时优先。
...
已死亡的实例若写过东西，它的 draft 会保留：重新入队的作业带上 `continue_from: <已死执行者>`，
这个标记既让它在该 draft 仍打开时可被领取，也让下一个启动实例接手它，于是崩溃那轮已经花掉的
调用得以续用，而不是重花一遍。已死亡实例若 draft 未被动过，则连同 draft 一起丢弃。
...
- **worker。** agent 执行器下，compile、evolve、episodes 作业交给同一个启动器（无人值守），
  ...
  index 写 L1 并入队片段判断（§5.12）；投影和重建仍是机械工作。领取时 index、投影、重建作业先于
  其他一切种类（L1 无条件可达，I3）；episodes 作业沿用其 index 作业在队列中的位置（`order_at`），
  因而先于该来源的编译；因 harness 未运行而重新入队的作业保留原来的位置。
- **两条道，两个 harness 进程。** 每用户**每道**一个在飞作业（`job_lanes.py`，architecture.md §5）：
  正本道装所有可能写库的种类——compile、evolve、evolve_adopt、groom、archive、challenge——规则与过去
  整个队列的规则一字不差；派生道装 index、episodes 和两种 recall 投影，它们从不打开正本。worker 同时
  排空两条道，各一个巡回循环，并在启动时说出来（`draining 2 lanes (canonical, derived)`）。于是一个库
  现在可能**同时有两个 harness 进程在外——每道至多一个**：一次编译轮次加一次片段判断，绝不会是两次编译。
  这个上界就是道的规则本身，而 worker 宁可拒绝同道内的第二次启动，也不去信任它（`_AGENT_ROUNDS`）。
  一个轮次所倚仗的一切都随之按道收窄：打开的草稿只占用自己那条道（所以 `pkc draft open` 不会被 worker
  正在跑的 episodes 轮次拒绝，而仍然会被另一次编译轮次拒绝）、启动时的 `attach_executor` 只看本道的草稿、
  领取所取的建议锁也按道设键，两条道因此不必轮流。冷却**不**按道分：订阅没额度是关于提供方的事实，
  所以它同时停下两条道上需要 harness 的种类，也不停下任何一条道的索引与投影。又因为队列顺序已无法
  再把一次编译拦在另一条道正在跑的作业后面，正本道会跳过那种其来源仍有 `index` 或 `episodes` 作业排队或在飞的编译——两种都算，
  因为那份 episodes 判断正是由 index 作业委派出来的；若剩下的编译全被这样挡住，这条道就空转。
- **一条认领熬不过一次中断。** 一个作业属于这个 worker 的那段时间，从 `claim_next` 把行交出来的那一刻起，
  到有人为它写下完结为止；drain 在整段时间里都记着自己手上拿的是什么（`_IN_FLIGHT`，每道一条），
  而不是去看抛出来的异常上写了什么。看异常正是线上失效的那一步：只有认领与完结这两处会带上 job id，
  于是一个 episodes 作业——它的轮次以 `exit 0` 干净收场，紧随其后的向量写入（轮次之后的那段活）
  撞上了消失两秒的 Qdrant——就一直 `claimed` 着，草稿也开着。它那条道再也认领不到东西，而因为每次编译都要等
  自己来源的派生工作，正本道也跟着空转：542 个作业挂起，半小时里什么也没完成，直到有人重启引擎。
  如今无论这段时间里抛出什么，这个作业要么被完结——当它的完结确实已经做完时，因为重跑一轮已经提交的编译
  会把同一批来源编译两遍——要么被重新入队；恢复那一行永远写明是哪一种（`(job <id> requeued)`、
  `(job <id> completed)`、`(no job in flight)`）：把这件事藏起来的正是沉默。除此之外，启动时的自愈也按时钟
  再跑（`WORKER_SELFHEAL_S`，60 秒），规则一字不差——活着的启动租约不动，正在执行的 `pkc` 命令的锁要让——
  只跳过本进程正在跑的那些认领：行上的任何证据都分不出它们与孤儿。于是别处没接住的泄漏只值一分钟，
  而不是一次重启。
