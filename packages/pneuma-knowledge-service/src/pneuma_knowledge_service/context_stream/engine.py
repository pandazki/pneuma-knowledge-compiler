"""The I/O half of AI cue: bind a session's `EvaluationPlan` to core + the app's ports.

Deliberately thin. Every decision worth testing is either in `session.py` (pure policy)
or in `pneuma_knowledge_core.recall.cue` (the contract and the four gates); what is left here is
...
from pneuma_knowledge_core.recall.cue import DETAIL_CONTRACT, CueResult, cue_once
...
def briefing_pack(system_prefix: str) -> str:
    ...
    (briefing.py). Core's cue engine takes `pack` as data for the HUMAN turn, and core
    ...
    into the middle of the cue Human turn: it tells the model it is a Q&A engine answering
    a owner's question, advertises `search_knowledge` / `fetch_verbatim` tools that are
    not bound on this call, and closes with the very 「无相关记录」 clause the cue contract
    exists to replace — the one that produces an empty card pushed onto the lens. The cue
    System contract would be arguing with a stale contract quoted underneath it.
...
async def run_evaluation(
    ...
    *,
    ...
    """One cue evaluation over a plan's window. `pack` given ⇒ briefing scope."""
    user = UserId(user_id)
    return await cue_once(
        user,
        plan.turns,
        as_of=as_of or datetime.now(timezone.utc),
        model=ctx.get_chat_model("cue"),
...
def _detail_human(cue: dict[str, Any], passages: Sequence[dict[str, Any]]) -> str:
    parts = [
        f"# 卡片\nkind: {cue.get('kind', '')}\n标题：{cue.get('title', '')}\n"
        f"正文：{cue.get('body', '')}\n触发片段：{cue.get('trigger', '')}"
    ]
    if passages:
        blocks = []
        for p in passages:
...
def _citations_of(cue: dict[str, Any]) -> list[Citation]:
    out: list[Citation] = []
    for raw in cue.get("citations") or []:
...
async def expand_cue(
    ctx: Any, user_id: str, cue: dict[str, Any]
) -> dict[str, Any]:
    ...
    response = await model.ainvoke(
        [
            ...
            HumanMessage(content=_detail_human(cue, passages)),
        ],
        config=invoke_config(
            "recall.cue_detail",
            **llm_call_config(
                ctx,
                operation="recall.cue_detail",
                user_id=user_id,
            ),
        ),
    )
    ...
    if not detail:
        ...
        # failed, and it contradicts the discipline the whole feature rests on — never push
        # an empty card onto the lens. The WS handler turns this into a non-fatal `error`
