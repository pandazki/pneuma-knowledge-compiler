...
**The archive** is how knowledge leaves every default retrieval without leaving the library: a canonical document is moved under the reserved root `archive/` (with its closed volumes; `git mv`, history intact), a source gets `archived_at` on L0. Both are authoritative marks; every derived index carries `archived` as a field re-derived on rebuild, every lane/listing takes `include_archived` (default false) and labels what it admits, the compile model never sees archived documents, and writes under `archive/` or at a shadowed live path are refused at the tool face and the gate (`archived_path`). An archived document leaves an **archive record** at its live path, written mechanically by the same commit (pure core `archive/record.py`, own narrow channel and own gate, no model): what the subject was, its span, how much it held, and the Owner's reason citing an `owner-dialogue/v1` statement the job ingests with `canonical_treatment: none`. The record is LIVE — glance, claims, every lane by default — and READ-ONLY to compile, so the subject goes on being answered *as archived* instead of vanishing. An archive is computed as a **proposal** (citation closure plus the record's facts, pure core `archive/proposal.py`), confirmed by the Owner against the same canonical HEAD, and executed by the worker's `archive` job. Design authority: [docs/design/archive.md](docs/design/archive.md). **The word `archive`/`归档` is reserved for this and nothing else.** Rollover's `<doc>/aNN.md` files are **closed volumes** of a work in several volumes (the page itself being the **open volume**) — live knowledge every lane reads; never call them archives. The only legacy spellings kept on disk are the frontmatter key `archived_from` and the fallback `type: archive` a volume carries, both stamped into every user's git library already.
