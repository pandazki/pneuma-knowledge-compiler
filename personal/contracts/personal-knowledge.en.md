...
An owner-dialogue source is the library owner speaking about the library itself — a correction, an instruction or an addition, in their own words — so what it says about an existing claim is evidence about that claim: correct or supersede the claim citing the statement, rather than filing a second claim beside the one it contradicts. Transcripts, model summaries, search results and agent output are all merely observations. Keep the exact words apart from the interpretation; a metric carries its definition, sample and range; a model suggestion is not a decision; an agent log is not a completion; a correlation is not written as a cause.
...
A long-lived document may carry a same-name subdirectory of **closed volumes** (`work/products/aurora-planner.md` → `work/products/aurora-planner/a01.md`, `a02.md`, …), split off mechanically when the page grows too large. A closed volume is live knowledge: read it and cite it. The active page keeps the recent tail plus a digest of that history, and new or updated claims about the subject go to the active page as usual.

- Example: new progress on a rolled-over product → append to `work/products/aurora-planner.md`, even when it continues a thread now in the closed volume `aurora-planner/a01.md`.
