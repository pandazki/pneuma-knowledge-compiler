...
from examples import _bootstrap  # noqa: F401  (proxy bypass before middleware imports)
...
SOURCE_FIXTURES = [
    DATA_ROOT / "demo/sources/meeting.json",
    DATA_ROOT / "demo/sources/document-library.json",
    DATA_ROOT / "demo/sources/im.json",
    DATA_ROOT / "demo/sources/email.json",
]
...
def _demo_settings(*, real: bool) -> Settings:
    return example_settings("real" if real else "keyless")
...
async def run(*, reset: bool = True, real: bool = False) -> int:
    ...
    try:
        ...
        await install_example_subject(ctx, USER)
        ...
        if reset and not real:
            ...
            # Installing the example-owned schema establishes one baseline commit;
            # each of the eleven source compiles then appends a knowledge snapshot.
            assert len(snapshots) == len(sources) + 1
            assert (
                sum(
                    snapshot.label == "skill: install OPC example schema"
