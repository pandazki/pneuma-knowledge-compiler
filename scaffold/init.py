...
#
# `compile` may also name an EXECUTOR instead of a model — `agent:codex` or
# `agent:claude-code` — and the round is then driven by that coding agent on this machine,
# under your own subscription, through the same draft and the same gate. Changing it takes a
# restart and governs future compiles only; an apply that moves the contract, the overlays,
# the components or the language re-installs the agent's skill, which reaches a session at
# its next start.
#
