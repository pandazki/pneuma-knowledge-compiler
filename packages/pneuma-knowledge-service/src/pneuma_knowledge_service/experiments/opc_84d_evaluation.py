...
def guarded_statement(text: str) -> bool:
    """Whether prose explicitly frames a statement as rejected/historical/uncertain."""
    normalized = unicodedata.normalize("NFKC", text).lower()
    return any(marker.lower() in normalized for marker in _GUARD_MARKERS)


def truth_entries(
    manifest: dict[str, Any], *, current_only: bool = False
) -> list[dict[str, Any]]:
