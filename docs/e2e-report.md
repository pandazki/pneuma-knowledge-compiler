...
hex 字面值 / 开源卫生 / git whitespace）全部干净；后端全套 pytest 425 通过；
...
- API：`scripts/dev-api.sh`，`127.0.0.1:18001`（uvicorn --reload），环境变量：
  ...
  - 注意：`llm_model` 基座一旦是 `scripted:` 会按 `wiring.resolve_model_name`
    覆盖所有角色路由，因此 cue 必须走 per-role env（`LLM_MODEL_CUE`）注入。
...
| 02 | 导入 Ingest | 初始态（文档/对话双 Tab、FilePicker、RadioGroup）；粘贴 synthetic markdown → 机械预览（2 blocks · 91 chars、section 树 b0–b1、提案 distill/full、双旋钮覆盖 Select）；未确认导入 | ✅ |
...
| 07 | 提示 Cue | SSE 一次性 Tab 初始态（转录窗口 + 评估参数 + 空态文案）；录入两轮 Atlas 发布进度转录、min_confidence 调 5 → 评估：存活 1/已下发 1（confidence 9 卡片 + 引用），GateLedger 门禁账 dropped：无引用 1 · 低置信 1 | ✅ |
...
| [02-ingest-initial.png](e2e/screenshots/02-ingest-initial.png) / [02-ingest-preview.png](e2e/screenshots/02-ingest-preview.png) | 导入初始态；机械预览（section 树 + proposed plan） |
...
| [07-cue-initial.png](e2e/screenshots/07-cue-initial.png) / [07-cue-gated.png](e2e/screenshots/07-cue-gated.png) | 提示初始态；评估后存活卡片 + GateLedger 门禁账 |
...
2. **Select 受控告警**。`ui/Select.tsx` 把 `null` 值映射为 `undefined`
   （Radix 的非受控），Cue 视图 focus 词表异步回填后从非受控跳到受控，React
...
| 检查 | 命令 | 结果 |
|---|---|---|
| 后端全套测试 | `uv run pytest -q` | **425 passed**（14.2s） |
...
1. **问答旅程无引用脚注**：scripted recall-script 的答案文本本身不含
   ...
   不是组件缺陷（recall/cue/library 的 Footnote 在各自旅程均验证正常）。
2. ~~390px 顶栏字标截断~~（已修复）：移动端字标收窄为 `Pneuma`，
   `· Knowledge Compiler` 副题从 sm 断点起显示，不再截断。
3. **scripted 模型游标一次性**：`ScriptedChatModel` 按进程内游标顺序回放，
   重跑 ask/cue 旅程前需重启 API 复位游标（e2e 脚本全量运行时序已按
   recall-script 3 回合 / cue-script 1 回合排好）。
