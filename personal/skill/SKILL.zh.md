...
5. 把这次使用交还给库：`pkc consult answer <handoff_id> --text-file -`（没跑 recall 时 `pkc consult record --question <q> --text-file -`，什么也没找到时加 `--kind no_record`）。recall 的交接在交出证据时已把问题和页面记为一条待作答的咨询，关闭补上回答；只读不经交接的会话，不记就对库不存在。
...
或用 `pkchome credentials set KEY` 保存密钥，再将选择设为 `on`。embedding 服务商的密钥
在写入前会先向该服务商验证一次，被拒绝的密钥不改变任何东西，原密钥仍然有效（离线时用
`--no-verify` 跳过验证保存）。保存密钥会自行重启运行中的引擎；
修改其他引擎设置后需重启。
