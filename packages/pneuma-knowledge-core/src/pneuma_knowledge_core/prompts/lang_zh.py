...
{kinds}
  当需求是**找人**——谁能来讲 X、谁懂 X——就把库里的查询对准 X **周围的人**，而不是 X 的定义：
  把 X 当作主体交给上面任何一条关于人的查询，另一条则花在同样照人来写的语义查询上
  （「做过 X 或同类工作的人」）——库里没有 X 本身时，正是这一条够得着最近的那份经验。
  这两条**就是**这类需求的计划。库里拿得出证据的、离它最近的那份经验，才是它的答案；
  把 X 解释一遍不是。
...
**答不上来的候选不是答案。** 说信息缺失、说无法确定、说需要它没有的访问权限——这样的文本没有
可交付的内容：它陈述的是一处**空缺**，而正文是「没有答案」的卡片，比不出卡更糟。宁可填 0。
交付出去的东西，必须给读者**添**上他原本没有的。

**有标记的近邻推荐是答案，装出来的匹配不是。** 当屋里在找「谁能做这件事、谁能来讲」，候选材料
真正佐证的那份最近的经验就回答了它——点出这个人，说清证据显示他在做什么，并**标明你这一步**：
这是库里最近的相关经验，不是关于那件事本身的记录。标明了，它有用；不标明，它就是在宣称一个库里
并不存在的匹配——那还是沾边。
...
_ZH: dict[str, str] = {
    ...
    "recall.live.discover.web_offer": (
        ...
        "——外部的新发布、产品、术语——才规划它；绝不用它替代先在库里找。当一个需求把内部的事"
        "和外部的主题混在一起——谁能来讲 X，而 X 是个公开产品——这条查询就指向 **X 本身**；"
        "找人那一半交给库里的查询。"
    ),
